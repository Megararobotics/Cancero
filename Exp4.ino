...
void setup() {
  ...
  Serial2.begin(9600);  // Initialize Bluetooth communication on Serial1
}
void loop() {
  // Wait for Bluetooth input
  if (Serial2.available() > 0) {
    char command = Serial2.read();  // Read from Bluetooth
...
void executeCommand(char command) {
  switch (command) {
    case 'B':  // Move Backward
      ...
      break;
    case 'F':  // Move Forward
      Serial2.println("Moving Forward");  // Send response to Bluetooth
      moveForward();
      break;
    case 'L':  // Move Left
      moveLeft();
...
void moveForward() {
  ...
  analogWrite(motor1PWM, 145);  // Adjust PWM value if needed
  analogWrite(motor2PWM, 165);  // Adjust PWM value if needed
...
void stopMotors() {
  ...
  Serial2.println("Robot stops moving");
}
